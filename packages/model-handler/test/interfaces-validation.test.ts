...
test('getErrorMessage', () => {
  ...
  ).toBe('Invalid value for index 1 of foo: expected foo but got bar')
  expect(
    getErrorMessage({ path: ['foo'], expected: 'foo', actual: 'bar' }),
  ).toBe('Invalid value for foo: expected foo but got bar')
})
...
describe('validateInterface()', () => {
  ...
  test('passes if the model views object is not empty', () => {
    expect(() => {
      validateInterface({
        views: { model: { did: { type: 'documentAccount' } } },
      } as unknown as ModelDefinitionV2)
    }).not.toThrow()
  })
})
...
describe('schema validation', () => {
  ...
  describe('validateBooleanSchema()', () => {
    test('handles const value', () => {
      ...
      expect(
        validateBooleanSchema(context, { const: false }, { const: true }),
      ).toEqual([
        { path: [], property: 'const', expected: false, actual: true },
      ])
      expect(validateBooleanSchema(context, {}, {})).toHaveLength(0)
      expect(
        validateBooleanSchema(context, { const: true }, { const: true }),
      ).toHaveLength(0)
    })
  })
  ...
  describe('validateNumberSchema()', () => {
    test('handles const value', () => {
      ...
      expect(
        validateNumberSchema(context, { const: 5 }, { const: 5 }),
      ).toHaveLength(0)
    })
    ...
    test('handles maximum value', () => {
      ...
      expect(
        validateNumberSchema(context, { maximum: 5 }, { maximum: 7 }),
      ).toEqual([{ path: [], property: 'maximum', expected: 5, actual: 7 }])
      expect(
        validateNumberSchema(context, { maximum: 5 }, { const: 7 }),
      ).toEqual([{ path: [], property: 'maximum', expected: 5, actual: 7 }])
      expect(validateNumberSchema(context, {}, { maximum: 5 })).toHaveLength(0)
      expect(
        validateNumberSchema(context, { maximum: 5 }, { maximum: 5 }),
      ).toHaveLength(0)
      expect(
        validateNumberSchema(context, { maximum: 5 }, { maximum: 3 }),
      ).toHaveLength(0)
      expect(
        validateNumberSchema(context, { maximum: 5 }, { const: 3 }),
      ).toHaveLength(0)
    })
    ...
    test('handles minimum value', () => {
      ...
      expect(
        validateNumberSchema(context, { minimum: 5 }, { minimum: 1 }),
      ).toEqual([{ path: [], property: 'minimum', expected: 5, actual: 1 }])
      expect(
        validateNumberSchema(context, { minimum: 5 }, { const: 1 }),
      ).toEqual([{ path: [], property: 'minimum', expected: 5, actual: 1 }])
      expect(validateNumberSchema(context, {}, { minimum: 5 })).toHaveLength(0)
      expect(
        validateNumberSchema(context, { minimum: 5 }, { minimum: 5 }),
      ).toHaveLength(0)
      expect(
        validateNumberSchema(context, { minimum: 5 }, { minimum: 6 }),
      ).toHaveLength(0)
      expect(
        validateNumberSchema(context, { minimum: 5 }, { const: 5 }),
      ).toHaveLength(0)
    })

    test('handles exclusiveMaximum value', () => {
      expect(
        validateNumberSchema(context, { exclusiveMaximum: 5 }, {}),
      ).toEqual([{ path: [], property: 'exclusiveMaximum', expected: 5 }])
      expect(
        validateNumberSchema(
          context,
          { exclusiveMaximum: 5 },
        ...
        { path: [], property: 'exclusiveMaximum', expected: 5, actual: 7 },
      ])
      expect(
        validateNumberSchema(context, { exclusiveMaximum: 5 }, { const: 5 }),
      ).toEqual([
      ...
      expect(
        validateNumberSchema(context, { exclusiveMaximum: 5 }, { const: 3 }),
      ).toHaveLength(0)
    })

    test('handles exclusiveMinimum value', () => {
      expect(
        validateNumberSchema(context, { exclusiveMinimum: 5 }, {}),
      ).toEqual([{ path: [], property: 'exclusiveMinimum', expected: 5 }])
      expect(
        validateNumberSchema(
          context,
          { exclusiveMinimum: 5 },
        ...
        { path: [], property: 'exclusiveMinimum', expected: 5, actual: 1 },
      ])
      expect(
        validateNumberSchema(context, { exclusiveMinimum: 5 }, { const: 5 }),
      ).toEqual([
      ...
      expect(
        validateNumberSchema(context, { exclusiveMinimum: 5 }, { const: 6 }),
      ).toHaveLength(0)
    })
  })
  ...
  describe('validateStringSchema()', () => {
    test('handles const value', () => {
      ...
      expect(
        validateStringSchema(context, { const: 'foo' }, { const: 'bar' }),
      ).toEqual([
        { path: [], property: 'const', expected: 'foo', actual: 'bar' },
      ])
      expect(validateStringSchema(context, {}, { const: 'foo' })).toHaveLength(
        0,
      )
      expect(
        validateStringSchema(context, { const: 'foo' }, { const: 'foo' }),
      ).toHaveLength(0)
    })

    test('handles enum value', () => {
      expect(
        validateStringSchema(context, { enum: ['a', 'b', 'c'] }, {}),
      ).toEqual([{ path: [], property: 'enum', expected: ['a', 'b', 'c'] }])
      expect(
        validateStringSchema(
          context,
          { enum: ['a', 'b', 'c'] },
      ...
      expect(
        validateStringSchema(context, {}, { enum: ['e', 'f'] }),
      ).toHaveLength(0)
      expect(
        validateStringSchema(
          context,
    ...
    test('handles pattern value', () => {
      expect(
        validateStringSchema(context, { pattern: '^[0-9]{2}$' }, {}),
      ).toEqual([{ path: [], property: 'pattern', expected: '^[0-9]{2}$' }])
      expect(
        validateStringSchema(
          context,
          { pattern: '^[0-9]{2}$' },
    ...
    test('handles maxLength value', () => {
      ...
      expect(
        validateStringSchema(context, { maxLength: 5 }, { maxLength: 7 }),
      ).toEqual([{ path: [], property: 'maxLength', expected: 5, actual: 7 }])
      expect(
        validateStringSchema(context, { maxLength: 5 }, { const: 'foobar' }),
      ).toEqual([
        { path: [], property: 'maxLength', expected: 5, actual: 'foobar' },
      ])
      expect(validateStringSchema(context, {}, { maxLength: 5 })).toHaveLength(
        0,
      )
      expect(
        validateStringSchema(context, { maxLength: 5 }, { maxLength: 5 }),
      ).toHaveLength(0)
      expect(
        validateStringSchema(context, { maxLength: 5 }, { maxLength: 3 }),
      ).toHaveLength(0)
      expect(
        validateStringSchema(context, { maxLength: 5 }, { const: 'foo' }),
      ).toHaveLength(0)
    })
    ...
    test('handles minLength value', () => {
      ...
      expect(
        validateStringSchema(context, { minLength: 5 }, { minLength: 1 }),
      ).toEqual([{ path: [], property: 'minLength', expected: 5, actual: 1 }])
      expect(
        validateStringSchema(context, { minLength: 5 }, { const: 'foo' }),
      ).toEqual([
        { path: [], property: 'minLength', expected: 5, actual: 'foo' },
      ])
      expect(validateStringSchema(context, {}, { minLength: 5 })).toHaveLength(
        0,
      )
      expect(
        validateStringSchema(context, { minLength: 5 }, { minLength: 5 }),
      ).toHaveLength(0)
      expect(
        validateStringSchema(context, { minLength: 5 }, { minLength: 6 }),
      ).toHaveLength(0)
      expect(
        validateStringSchema(context, { minLength: 5 }, { const: 'foobar' }),
      ).toHaveLength(0)
    })
  })
  ...
  describe('validateArraySchema()', () => {
    test('items schema must be defined', () => {
      expect(
        validateArraySchema(context, {}, { items: { type: 'string' } }),
      ).toEqual([
        {
          path: [],
          property: 'items',
    ...
    test('handles items schema', () => {
      ...
      expect(
        ...
        { path: ['items'], property: 'title', expected: 'Foo', actual: 'Bar' },
      ])
      expect(
        validateArraySchema(
          { ...createResolvers(expected, {}), path: [] },
      ...
      ).toEqual([
        {
          path: [],
          property: 'items',
          expected: { title: 'Foo', type: 'string' },
          actual: null,
        },
      ])
  ...
  describe('validateObjectSchema()', () => {
    test('handles expected required properties', () => {
      ...
      ).toEqual([{ path: [], property: 'required', expected: 'foo' }])
      expect(
        validateObjectSchema(
          context,
          { required: ['foo'] },
      ...
      expect(
        validateObjectSchema(context, {}, { required: ['foo'] }),
      ).toHaveLength(0)
    })
  ...
  test('validateAllSchemaTypes()', () => {
    ...
    expect(
      validateAllSchemaTypes(context, expectedList, [{ type: 'string' }]),
    ).toEqual([{ path: [], property: 'length', expected: 3, actual: 1 }])
    expect(
  ...
  test('validateAnySchemaTypes()', () => {
    ...
    expect(
      validateAnySchemaTypes(context, expectedList, [{ type: 'boolean' }]),
    ).toEqual([
  ...
  describe('validateSchemaType()', () => {
    ...
    test('validates type match', () => {
      expect(
        validateSchemaType(context, { type: 'string' }, { type: 'number' }),
      ).toEqual([
        { path: [], property: 'type', expected: 'string', actual: 'number' },
      ])
      expect(
        validateSchemaType(context, { type: 'string' }, { type: 'string' }),
      ).toHaveLength(0)
    })
    ...
    test('validates title match when provided', () => {
      ...
      expect(
        ...
        { path: [], property: 'title', expected: 'foo', actual: 'bar' },
      ])
      expect(
        validateSchemaType(
          context,
    ...
    test('validates allOf schemas', () => {
      ...
      expect(
        ...
        { path: ['allOf'], property: 'length', expected: 2, actual: 1 },
      ])
      expect(
        validateSchemaType(
          context,
    ...
    test('validates oneOf schemas', () => {
      ...
      expect(
        ...
        { path: ['oneOf'], property: 'length', expected: 2, actual: 1 },
      ])
      expect(
        validateSchemaType(
          context,
    ...
    test('supports null schemas', () => {
      expect(
        validateSchemaType(context, { type: 'null' }, { type: 'null' }),
      ).toHaveLength(0)
    })
  ...
  test('validateSchemaImplementation()', () => {
    ...
    expect(
      ...
      { path: ['foo'], property: 'type', expected: 'string', actual: 'number' },
    ])
  ...
  test('validateInterfaceImplementation() validates a model implementation of an interface', () => {
    ...
    try {
      ...
      expect(error.message).toBe(
        'Invalid implementation of interface interfaceID',
      )
      ...
      expect(error.errors[2].message).toBe(
        'Invalid views implementation of interface interfaceID',
      )
    }
